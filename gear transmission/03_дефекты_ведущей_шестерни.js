...
function init() {
  ...
  set_name("Дефекты ведущей шестерни");
...
function display() {
  ...
  for (i = 1; i <= 20; i++) ausp.harms[0].add((i * f) / 2, 1, 1, 0);
  for (i = 1; i <= 20; i++) spen.harms[0].add((i * f) / 2, 1, 1, 0);
  ...
  for (i = 1; i <= 3; i++)
    ausp.harms[1].add_modulated(i * fz(f), 1, f, 1, 4, 3, 1, 1, 0);

  for (i = 1; i <= 3; i++)
    spen.harms[1].add_modulated(i * fz(f), 1, f, 1, 3, 3, 1, 1, 0);
...
function diagnostic() {
  ...
  // 1-3 - аналогично get_cnt_harms(int idx, unsigned int from, int fails);
  // 4 - необходимое кол-во присутствующих модуляций;
  // 5 - истина - с двух сторон, ложь - достаточно с одной;

  var cnt_harms_ausp_1 = ausp.get_cnt_harms(1, 1, 1, 2, true); // с обоих сторон
  console.log("AUSP harms count (fz): " + cnt_harms_ausp_1);

  var cnt_harms_spen_1 = spen.get_cnt_harms(1, 1, 1, 2, true); // с обоих сторон
  console.log("SPEN harms count (fz): " + cnt_harms_spen_1);

  var cnt_harms_ausp_1_m = ausp.get_cnt_harms(1, 1, 1, 2, false); // достаточно с одной стороны
  console.log("AUSP harms count (fz): " + cnt_harms_ausp_1_m);

  var cnt_harms_spen_1_m = spen.get_cnt_harms(1, 1, 1, 2, false); // достаточно с одной стороны
