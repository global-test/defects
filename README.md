...
Данные объекты доступны в любой функции скрипта. У объектов ``ausp`` и ``spen`` доустпно две группы методов. 
...
Методы из второй группы применяются в йункции ``diagnostic()``. 
...
* ``.get_slope(N);`` - возвращает логическую переменную, котороая может принимать значения ``true`` или ``false``, что обозначает обнаружен или нет наклон. Параметр ``N`` - индекс запрашиваемого диапазона.
