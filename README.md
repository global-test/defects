...
Этот объект предназначен для получения из приложения GTLd данных о сигнале. Поддерживает следующие методы:
...
Данные объекты доступны в любой функции скрипта. У объектов ``ausp``, ``ausp_hf``, ``spen`` и ``spen_hf`` доступно две группы методов. 
...
### Методы настройки спектров ``ausp``, ``ausp_hf``, ``spen`` и ``spen_hf`` (методы группы №1):
<br><br>
* ``.set_options(FREQ_BEGIN, FREQ_END, DELTA)`` - установка частотных настроек спектра, ``FREQ_RANGE`` - частотный диапазон, ``POINT_COUNT`` - количество точек спектра, ``S1`` - количество устреднений, ``S2`` - количество сглаживаний средней линии; 
### Пример: ``ausp.set_options(1000, 1000, 5, 15);`` // пример для автоспектра
<br><br>
* ``.add(FREQ, WIDTH, COLOR_INDEX)`` - добавление гармоник вибрационного портрета на спектр. Первый параметр ``FREQ`` - частота, второй `WIDTH` - толщнина линии, третий - индекс колор-бокса, задающего цвет; 
...
<br><br>
* ``.set_filter(FREQ, WIDTH)`` - настройка полосового фильтра для спектра. Параметр ``FREQ`` - частота,  ``WIDTH`` - ширина полосы;
