...
* ``.get_cnt_harms(N);`` - возвращает количество гармоник, присутствующих на автоспектре. Параметр ``N`` - допустимое количество пропущенных гармоник в ряду;
...
<br><br>
* ``.get_slope(N);`` - возвращает логическую переменную, котороая может принимать значения ``true`` или ``false``, что обозначает обнаружен или нет наклон. Параметр ``N`` - 
