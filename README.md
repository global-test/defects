...
добавлены объекты object и parent. 
object.get_type() – возвращает тип;
object.get_parameter("internalD") – возвращает значение параметра;
для парента методы те же.
но оказалось, этого мало для бесконечной иерархии, т.к. для корректного расчета частоты необходимо знать всех предков, а не одного последнего.
Видится два варианта как это преодолеть:
1. object.get_parents() – вернет массив все предков;
2. object.parent() – вернет объект родителя объекта, можно будет получить всех предков рекурсивно. Но пока нет полной ясности как это сделать).
...
4. [Глобальные объекты](#global_objects)
    ...
    * [ausp, ausp_hf, spen и spen_hf](#spectrs) 
        * [Методы спектральных объектов](#common_methods)
            + [set_harms_count](#set_harms_count)
            + [set_slopes_count](#set_slopes_count)
            + [set_humps_count](#set_humps_count)
            + [set_options](#set_options)
            + [set_filter](#set_filter)
        * [Дочерние массивы спектральных объектов](#common_arrays)
            + [set_options](#set_options)
            + [set_filter](#set_filter)
5. [Массив наборов гармоник спектальных объектов - harms](#harms_array)
    * [Методы настройки harms](#harms_methods)
        + [add](#harms_add)
        + [add_modulated](#harms_add_modulated)
        + [set_declay](#set_declay)
    * [Методы извлечения данных из harms](#harms_output)
...
6. [Массив наборов наклонов спектальных объектов - slopes](#slopes_array)
    * [Методы настройки slopes](#slopes_methods)
        + [add_slope](#add_slope)
    * [Методы извлечения данных из slopes](#slopes_output)
        + [get_slope](#get_slope)
7. [Массив наборов наклонов спектальных объектов - humps](#humps_array)
    * [Методы настройки humps](#humps_methods)
        + [add_hump](#add_hump)
    * [Методы извлечения данных из humps](#humps_output)
        + [get_hump](#get_hump)
...
## <a name="common_methods"></a>Методы спектральных объектов
...
### Пример: ``ausp.set_slopes_count(2);`` // пример для автоспектра
...
### Пример: ``ausp.set_humps_count(3);`` // пример для автоспектра

<hr><br><br>

## <a name="set_options"></a>Метод ``.set_options(FREQ_BEGIN, FREQ_END, DELTA)``
Предназначен для установки частотных настроек спектра, ``FREQ_RANGE`` - частотный диапазон, ``POINT_COUNT`` - количество точек спектра, ``S1`` - количество устреднений, ``S2`` - количество сглаживаний средней линии; 
### Пример: ``ausp.set_options(1000, 1000, 5, 15);`` // пример для автоспектра

<hr><br><br>

## <a name="set_filter"></a>Метод ``.set_filter(FREQ, WIDTH)``
Предназначен для настройки полосового фильтра для спектра. Параметр ``FREQ`` - частота,  ``WIDTH`` - ширина полосы;
### Пример: ``spen.set_filter(fc, (fc * 2) / 3);`` // пример для спектра огибающей

<hr><br><br>

## <a name="child_objects"></a>Дочерние массивы спектральных объектов
...
## <a name="harms_methods"></a> Методы настройки массива ``harms[]``
...
## <a name="harms_add_modulated"></a>Метод ``.harms[N].add_modulated(FREQ, M_FREQ, COUNT, WIDTH, COLOR_INDEX, M_WIDTH, M_COLOR_INDEX)``	
...
## <a name="is_harms_declay"></a>Метод ``.harms[N].is_harms_declay()``
Возвращает логическую переменную, которая может принимать значения ``true`` или ``false``, что означает обнаружено или нет затухание гармоник. Параметр ``N`` задаёт индекс набора гармоник.
### Пример: ``var is_harms_ausp_declay = ausp.harms[1].is_harms_declay();``

<hr><br><br>

# <a name="slopes_array"></a>6. Массив наборов наклонов спектральных объектов - ``slopes[]``

Каждый элемент массива ``slopes[]`` представляет собой объект для работы с набором наклонов, в ходе анализа возможна настройка произвольного количества наборов наклонов, используемого в объекте спектра. Доступ к набору наклонов осуществляется следующим образом: ``ausp.slopes[N]``, где ``N`` - индекс набора наклонов. 

## <a name="slopes_methods">Методы настройки массива ``slopes[]``
## <a name="add_slope"></a>Метод ``.slopes[N].add_slope(FREQ_BEGIN, FREQ_END, COLOR_INDEX)`` 
Предназначен для передачи параметров поиска наклона спектра. Где ``N`` - индекс набора наклонов. Параметры ``FREQ_BEGIN`` и  ``FREQ_END`` - начало и конец частотного диапазона, ``COLOR_INDEX`` - индекс цвета.
### Пример: ``spen.slopes[0].add_slope(0, freq / 2, 0);`` // пример для спектра огибающей

<hr><br><br>

## <a name="slopes_output"></a>Методы извлечения результатов из ``slopes[]``
## <a name="get_slope"></a>Метод ``.slopes[N].get_slope(I)``
Возвращает логическую переменную, которая может принимать значения ``true`` или ``false``, что означает обнаружен или нет наклон. Где ``N`` - индекс набора наклонов. Параметр ``I`` - индекс запрашиваемого диапазона.
### Пример: ``var slope_ausp = ausp.slopes[5].get_slope(0);``

<hr><br><br>

# <a name="slopes_array"></a>7. Массив наборов наклонов спектральных объектов - ``humps[]``

Каждый элемент массива ``humps[]`` представляет собой объект для работы с набором горбов, в ходе анализа возможна настройка произвольного количества наборов горбов, используемого в объекте спектра. Доступ к набору горбов осуществляется следующим образом: ``ausp.humps[N]``, где ``N`` - индекс набора наклонов. 

## <a name="humps_methods">Методы настройки массива ``humps[]``
## <a name="add_hump"></a>Метод ``.humps[N].add_hump(FREQ_BEGIN, FREQ_END, COLOR_INDEX)``
Предназначен для передачи параметров поиска горбов спектра. Где ``N`` - индекс набора горбов. Параметры ``FREQ_BEGIN`` и  ``FREQ_END`` - начало и конец частотного диапазона, ``COLOR_INDEX`` - индекс цвета.
### Пример: ``spen.humps[1].add_hump(5000, 10000, 0);`` // пример для спектра огибающей

<hr><br><br>

## <a name="humps_output"></a>Методы извлечения результатов из ``humps[]``
## <a name="get_hump"></a>Метод ``.humps[N].get_hump(I)`` 
Возвращает логическую переменную, которая может принимать значения ``true`` или ``false``, что означает обнаружен или нет горб на спектре. Параметр ``I`` - 
индекс запрашиваемого диапазона. Где ``N`` - индекс набора горбов.
### Пример: ``var hump_ausp = ausp.humps[0].get_hump(0);``
