...
# <a name="harms"></a>6. Массив наборов наклонов спектральных объектов - ``slopes[]``
...
Предназначен для передача параметров поиска наклона спектра. Параметры ``FREQ_BEGIN`` и  ``FREQ_END`` - начало и конец частотного диапазона, ``COLOR_INDEX`` - индекс цвета;
