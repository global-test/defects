...
## <a name="add_hump"></a>Метод ``.add_hump(FREQ_BEGIN, FREQ_END, COLOR_INDEX)``
Предназначен для передачи параметров поиска горбов спектра. Параметры ``FREQ_BEGIN`` и  ``FREQ_END`` - начало и конец частотного диапазона, ``COLOR_INDEX`` - индекс цвета.
