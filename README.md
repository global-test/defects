...
## <a name="get_cnt_harms"></a>Метод ``.get_cnt_harms(N)``
Возвращает количество гармоник, присутствующих на спектре. Параметр ``N`` - допустимое количество пропущенных гармоник в ряду;
### Пример: ``var cnt_harms_ausp = ausp.get_cnt_harms(2);`` 

<hr><br><br>

## <a name="is_harms_declay"></a>Метод ``.is_harms_declay()``
Возвращает логическую переменную, которая может принимать значения ``true`` или ``false``, что означает обнаружено или нет затухание гармоник;
...
## <a name="add_slope"></a>Метод ``.add_slope(FREQ_BEGIN, FREQ_END, COLOR_INDEX)`` 
Предназначен для передачи параметров поиска наклона спектра. Параметры ``FREQ_BEGIN`` и  ``FREQ_END`` - начало и конец частотного диапазона, ``COLOR_INDEX`` - индекс цвета;
...
## <a name="get_slope"></a>Метод ``.get_slope(N)``
Возвращает логическую переменную, которая может принимать значения ``true`` или ``false``, что означает обнаружен или нет наклон. Параметр ``N`` - 
