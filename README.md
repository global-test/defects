# DEFECTS - подпроект GTLd (версия 1)
...
Применяется в функции ``init``. Предназначена для добавление колор-бокса, применяется в функции ``init``. Параметр``COLOR`` - цвет, ``LABEL`` - обозначение
...
#### Пример использования:
return_result(false, "дефект не обнаружен");
...
#### Пример использования:
``ausp.add(i*freq, 1, 0);`` // для автоспектра
...
#### Пример использования:
``spen.add_hump(5000, 10000, 0);`` // для спектра огибающей
