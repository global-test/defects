...
<br><br>
* ``.is_harms_decay();``- возвращает логическую переменную, которая может принимать значения ``true`` или ``false``, что означает обнаружено или нет затухание гармоник;
### Пример: ``var is_harms_ausp_decay = ausp.is_harms_decay();``
<br><br>
