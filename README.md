...
Данные объектны досутпны в люой функции скрипта, но нобор методов логически разбит на две группы. В первой группе находятся методы настройки спектров, они применяются в функции ``display()`` и ``init()``. Во второй группе находятся методы извлечения результатов анализа спектров. Методы из второй группы применяются в йункции ``diagnostic()``. Ниже представлены методы настройки спектров и методы извлечения результатов анализа спектров. 
