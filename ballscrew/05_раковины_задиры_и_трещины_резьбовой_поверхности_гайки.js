...
function init() {
  ...
  set_name("Раковины, задиры и трещины резьбовой поверхности гайки");

  // Добавление колорбокса. первый параметр - цвет, второй - текст
  add_color(0xff004dff, "Fгк");
  add_color(0xff004dff, "±Fврв");
...
function display() {
  ...
  for (i = 1; i <= 7; i++) ausp.harms[0].add(i * f_nut, 1, 1, 0);

  // Добавление гармоник на спектр огибающей.
  // harms_spen - список отображаемых на спектре огибающей гармоник.
  // 1 - несущая частота
  // 2 - вес
  // 3 - модулирующая
  // 4 - вес модулирующей
  ...
  for (i = 1; i <= 7; i++)
    spen.harms[0].add_modulated(i * f_nut, 1, freq, 1, 1, 3, 0, 1, 1);
